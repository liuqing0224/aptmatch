...
export const api = {
  ...
  resumes: {
    ...
    parse: (file: File) => {
      ...
      return request<{ name: string; text: string }>('/api/resumes/parse', {
        method: 'POST',
        body: form,
      });
    },
  ...
  positions: {
    list: () => request<{ positions: Company[] }>('/api/positions').then((r) => r.positions),
    create: (body: { name: string; feishu_doc_url?: string; jd_text?: string }) =>
      request<{ position: Company }>('/api/positions', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }).then((r) => r.position),
    remove: (id: string) =>
      request<{ ok: boolean }>(`/api/positions/${id}`, { method: 'DELETE' }),
  },
  candidates: {
    list: (params?: { position_id?: string; status?: string }) => {
      const qs = new URLSearchParams();
      if (params?.position_id) qs.set('position_id', params.position_id);
      if (params?.status) qs.set('status', params.status);
      const suffix = qs.toString() ? `?${qs.toString()}` : '';
      return request<{ candidates: Candidate[] }>(`/api/candidates${suffix}`).then(
        (r) => r.candidates
      );
    },
    importFeishu: (body: {
      position_id: string;
      agent_id?: string;
      candidates: { name: string; text: string; source_url: string }[];
    }) =>
      request<{
        imported: { id: string; resume_id: string; name: string; source_url: string }[];
        skipped: number;
        dispatched: string[];
        message: string;
      }>('/api/candidates/import-feishu', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }),
    collectStart: (body: { position_id: string }) =>
      request<{ task: Task }>('/api/candidates/collect-start', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(body),
      }).then((r) => r.task),
