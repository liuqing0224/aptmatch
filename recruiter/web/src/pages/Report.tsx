  ...
  return (
    <div>
      <div className="page-head">
        <div>
          <Link to="/?tab=candidates" className="back-link">← 返回候选人库</Link>
          <h1>{task.title}</h1>
        </div>
      ...
      {blkHits.length > 0 && (
        <div className="alert alert-error" style={{ marginBottom: 16 }}>
          ...
          上榜原因：{blkHits[0].issue || '详见来源'}。请在面试前重点核实。
        </div>
      )}
      ...
      {collectResult && (
        <div className="card">
          ...
          {collectResult.ok === false ? (
            <div className="alert alert-error">{collectResult.message || '采集失败，请查看任务日志'}</div>
          ) : (
            <>
              <p>{collectResult.message || '采集完成'}</p>
              <div className="row">
                <span>导入：{collectResult.imported ?? 0}</span>
                <span>跳过：{collectResult.skipped ?? 0}</span>
                <span>派发筛选：{collectResult.dispatched ?? 0}</span>
              </div>
            </>
          )}
          <div className="muted" style={{ marginTop: 8 }}>
            采集的候选人已进入「招聘端 → 候选人库」，筛选完成后自动获得评分/等级/摘要。
          </div>
        </div>
      )}
